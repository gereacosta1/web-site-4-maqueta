...
type Props = {
  ...
  totalUSD?: number;        // opcional; si no viene lo calculamos de items
  shippingUSD?: number;     // opcional
  taxUSD?: number;          // opcional
  customer?: Customer;      // opcional; si no viene usamos fallbacks válidos
};
  ...
  return (
    <div
      ...
      className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-[9999] px-4 py-3 rounded-xl shadow-2xl border text-sm font-semibold ${
        type === 'success' ? 'bg-green-600/95 text-white border-green-400'
          : type === 'error' ? 'bg-red-600/95 text-white border-red-400'
...
}: { open: boolean; title: string; children: ReactNode; primaryLabel?: string; onPrimary?: () => void; secondaryLabel?: string; onClose: () => void; }) {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-[9998] flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-[95%] max-w-md p-6">
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          {primaryLabel && (
            <button onClick={onPrimary} className="px-4 py-2 rounded-lg bg-black text-white font-bold hover:bg-gray-900">
              {primaryLabel}
            </button>
          )}
        </div>
...
export default function AffirmButton({
  ...
  useEffect(() => {
    loadAffirm(PUBLIC_KEY, ENV).then(() => setReady(true)).catch((e) => {
      console.error(e);
      setReady(false);
    });
  }, []);

  // Mapear items del carrito a formato Affirm
  ...
  async function handleClick() {
    ...
    if (!affirm?.checkout) {
      showToast('error', 'Affirm no está listo');
      return;
    }
    if (!canPay) {
      setModal({ open: true, title: 'Importe no disponible', body: 'El total es demasiado bajo para Affirm.', retry: false });
      return;
    }
    ...
    console.table(checkout.items.map((it: any) => ({ name: it.display_name, sku: it.sku, cents: it.unit_price, qty: it.qty })));
    console.log('TOTAL cents:', checkout.total, 'shipping:', checkout.shipping_amount, 'tax:', checkout.tax_amount);
    console.groupEnd();
    ...
    try {
      ...
      affirm.checkout.open({
        onSuccess: async ({ checkout_token }: { checkout_token: string }) => {
          // Lado servidor opcional (ya tenés /netlify/functions/affirm-authorize.mjs)
          try {
